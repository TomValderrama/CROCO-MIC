...
%%
cd .. 
ax=figure('Units','characters','Position',[-2.666000000000000e+02,-0.923076923076923,266,77.461538461538470])
	pcolor(lon,lat,zetalvl); shading interp; colormap(jet(64))
        ax = gca;
ax.FontSize = 20; 
...
%     caxis([-1.6 1.6])
	...
	title('Map of tidal stations','FontSize',40,'FontWeight','bold')
	axis([-76 -72 -47 -40])
	hold on
		plot(lonBGQ,latBGQ,'m.','MarkerSize',30)
hcb = colorbar;
hcb.Title
hcb.Title.String = "[m]";
		for k=1:size(lonBGQ,2)
			text(lonBGQ(k),latBGQ(k),num2str(k),'FontSize',15,'FontWeight',	'bold')
		end
	hold off
%%
