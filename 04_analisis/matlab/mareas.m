...
%
nc=netcdf('mosa_BGQ_his_M1_1h_WD_20210102.nc','r');  % Simon's Town, South Africa
...
time=nc{'time'}(:);    %Days since 17000101 / Gregorian   % fails if the whole series is used, why?
depth=nc{'h'}(:);  % m
lat=nc{'lat_rho'}(:);
lon=nc{'lon_rho'}(:);
zeta=nc{'zeta'}(:); % in mm. Bad = -32768
id=nc.id(:);
...
zeta=zeta(:,490,282); % lat= -41.4948, lon= -72.9750
 
% depth=depth(489,282);

% zeta=reshape(zeta,1,1,721);
slvl=zeta;
% slvl=zeta+depth;

% slvl=reshape(slvl,721,1,1);
...
title([name 'lat= -41.4948, lon= -72.9750, ID: ',id])
xlabel('Fecha')
...
%
  ...
  title(['lat= -41.4948, lon= -72.9750, Datos ' name])
...
title (['lat= -41.4948, lon= -72.9750, Nivel del Mar (m) ' name ]);
...
plot(fecha,XOUT,'b')
title('lat= -41.4948, lon= -72.9750, Marea Astronomica (m)')
%xlabel('Julian Time (days)')
ylabel('NMM_a (m)')
datetick('x','yyyy-mm')
axis tight
...
plot(fecha,meteo,'r')
title('lat= -41.4948, lon= -72.9750, Marea Meteorologica (m)')
